...
- checkItem - проверяет наличие товара в корзине и предает кнопке соответсвующее значение "В корзину/Убрать с корзины";
...
#### Класс Order
Отвечает за отображение окна оформления заказа. Класс используется для отображения кнопки выбора способы оплаты и полей ввода адреса и контактных данных. В конструктор класса передается `HTML`-элемент и экземпляр `EventEmitter` для инициации событий.\
- _paymentButton - элемент кнопки выбора способа оплаты;

Методы:
- set - устанавливает значение адреса, номера телефона и почты из полей ввода;
